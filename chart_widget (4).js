...
function renderChartSVG(c,h,l,v,t,tf,pre,renko,renkoBrick,mlExit){
  ...
  updateInfoBar(c,h,l,v,kama,sarArr,sarBull,ao,mlExit,segnalePerBar,bafC);
...
function updateInfoBar(c,h,l,v,kama,sarArr,sarBull,ao,mlExit,segnalePerBar,bafC){
  ...
  const aoArr=[...ao].filter(v=>v!=null);
  const aoImproving=aoArr.length>=2&&aoArr[aoArr.length-1]>aoArr[aoArr.length-2];
  // Baffetti: stessa fonte di verità usata per marker/storia (bafC = pre.baff se disponibile).
  // calcBaf(ao) locale è una definizione diversa (rialzi consecutivi dell'AO, non del prezzo
  // midpoint) e va usata solo come fallback quando manca il dato precalcolato.
  const baff=(bafC && bafC.length) ? bafC[bafC.length-1] : calcBaf(ao);
  ...
  // Fonte di verità: ultimo segnale precalcolato dal motore Python (segnalePerBar già
  // preferisce pre.segnale quando disponibile). Fallback su calcSegnale() solo se assente.
  let segnale = null;
  if (segnalePerBar) {
    for (let i = segnalePerBar.length - 1; i >= 0; i--) {
      if (segnalePerBar[i]) { segnale = segnalePerBar[i]; break; }
    }
  }
  if (!segnale) segnale = calcSegnale(c,kama,sarVal,sarIsBull,ao,er,baff,mmAlign,rsi||50);
