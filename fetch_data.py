...
def process_ticker(info, regime_mult, regime_name):
    ...
    try:
        ...
        live = {
            ...
            'segnale': segnale_arr[-1], 'zona': zona, 'score': score,
            'prezzo': round(lc,4),
            'kama_fast': round(kf,4) if kf else None,
            'kama_slow': round(ks,4) if ks else None,
            'er': er_arr[-1], 'baff': baff_arr[-1], 'ao': ao_arr[-1],
            'rsi': rsi_arr[-1], 'rsi5': rsi5_arr[-1],
            'volRatio': vol_r_arr[-1],
            'sar': round(sar_arr[-1],4) if sar_arr[-1] else None,
            'sarBull': sar_bull_arr[-1],
            'entryDate': entry_date,
            'perfOggi':  round((lc/close[-2]-1)*100,2)  if len(close)>2  else 0,
            'perfSett':  round((lc/close[-6]-1)*100,2)  if len(close)>6  else 0,
            'perfMese':  round((lc/close[-21]-1)*100,2) if len(close)>21 else 0,
        }

        # ── file chart per il widget scannerv3 ──
        bars = [[ts[i], round(openp[i],4), round(high[i],4), round(low[i],4),
                 round(close[i],4), int(volume[i])] for i in range(len(close))]
        chart = {
            'sym': symbol, 'ticker': info['t'], 'nome': info.get('n',''),
            'd': bars,
            'kama_d': [round(v,4) if v is not None else None for v in kama_fast],
            'kamaSlow_d': [round(v,4) if v is not None else None for v in kama_slow],
            'sar_d': [round(v,4) if v is not None else None for v in sar_arr],
            'sarBull_d': sar_bull_arr,
            'ao_d': [round(v,4) for v in ao_arr],
            'rsi_d': rsi_arr, 'rsi5_d': rsi5_arr,
            'baff_d': baff_arr, 'er_d': er_arr,
